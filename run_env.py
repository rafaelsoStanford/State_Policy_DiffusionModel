...
for i in range(max_steps):
  ...
  position =  pos_hist[:obs_horizon,:]
  action = np.stack(x['action'] for x in observation_deque)
  ...
  obs_cond = torch.cat((normalized_position, normalized_actions, image_features), dim=-1) # (B, 1, t_0:t_obs , 2+3+128)
...
unnormalized_position = unnormalize_data(position_pred, position_stats)
